...
const HANDLES = Object.freeze([
  ...
  (pos, dim, opts) => {
    ...
    else if (dim.y2 + dim.x2 - x2 > dim.h) {
  ...
  (pos, dim, opts) =>
    (dim.y =
      dim.y2 - pos.y < opts.min_crop_height
        ? dim.y2 - opts.min_crop_height
        : pos.y),
  //  RIGHT
  (pos, dim, opts) =>
    (dim.x2 =
      pos.x - dim.x < opts.min_crop_width
        ? dim.x + opts.min_crop_width
        : pos.x),
  //  BOTTOM
  (pos, dim, opts) =>
    (dim.y2 =
      pos.y - dim.y < opts.min_crop_height
        ? dim.y + opts.min_crop_height
        : pos.y),
  //  LEFT
  (pos, dim, opts) =>
    (dim.x =
      dim.x2 - pos.x < opts.min_crop_width
...
export default class Handle {
  constructor(parent, type, scope) {
    this.$$view = cell(
      ...
      [
        'imgc-handles-el',
        `imgc-handles-el-${Math.floor(type / 4)}-${type % 4}`,
      ],
      {},
      parent
    )
